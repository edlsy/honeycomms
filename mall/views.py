...
def mall_product_list(request):
    ...
    ktshop_device_url_template = "https://shop.kt.com/smart/productView.do?prodNo=&vndrNo=&supportType=02"
    ...
    for idx, thumbs in enumerate(thumbs_blocks):
        ...
        if len(Product.objects.filter(device_name=item_name[idx], device_price=item_price[idx]))==0:
            ...
            if len(Product.objects.filter(device_name=item_name[idx])) == 0:
                print("%s info saving...\n", item_name[idx])
                temp_ktshop_link = ktshop_device_URL(ktshop_device_url_template, item_code[idx], 48, vndr_code, 56)
                print("ktshop_link : ", temp_ktshop_link)
                Product(device_name=item_name[idx], device_price=item_price[idx], device_code=item_code[idx], ktshop_link=temp_ktshop_link, img_link=local_img_path_in_template+item_name[idx]+".png").save()
